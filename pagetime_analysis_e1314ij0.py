...
page_time = pd.read_csv(file_location + 'Raw_otree/PageTimes-e1314ij0.csv')
...
len(participant_code) # 46 total participants
...
waiting_label = ['663a94c4f1f80e7c9c8a2d19',
'6723a12849796982b39f3c28',
'663d8f0edfd87b4c1f590b99',
'662945728093113a43ab8077',
'5c747b0f181ad300113363ed',
'610008383c0a128712403745',
'64275e53f51a5ed4f6b7d019',
'675cf75e3c237ff931f4a43e',
'63251e479509813fab713ad2',
'63e548363c6296f44832c59c',
'6718343c82dda1264ded40d6',
'5753046ba6deb50006d64b0f',
'673674170c987b500401215f',
'5de7b588ff15c2a06658dd1d',
'67e1613d4ddf1135fbb48d52',
'674780197d2f877d208e64bf',
'5b5df59dd113f600012de8e9',
'6546e4c359e7f644c638df33']
                 

base_pay_label = ['5c0b388b9c7b1100015ab55f',
'672268205351c30413c9837e',
'671ab0904de8eb7273ae9d42',
'5c1c2c51e2d9150001b5737e',
'6757069f8e2b810b4fabd66b',
'67ebe6d62f216f4e47fcc5e0',
'666cac5ed30140cf2b1e019c',
'6118b3bf8ec743208bae28c8',
'63e584009bf1aa55a39e1d53',
'5e07a0898dffa00ff1aaf60f']

grouped_participants_label = [
'6723a12849796982b39f3c28',
'662945728093113a43ab8077',
'663a94c4f1f80e7c9c8a2d19',
'5c747b0f181ad300113363ed',
'64275e53f51a5ed4f6b7d019',
'610008383c0a128712403745',
'675cf75e3c237ff931f4a43e',
...
len(base_pay_label) # 10 base pay participants
len(waiting_label) # 18 waiting participants
len(grouped_participants_label)

nocode_label = ['652359572fd30ff57dfea7fb',
'674d1412e0b30ad8c359b635',
'673d557afaf168a9beecc6d1']

# match the participant code with Prolific participant label
prolific_data = pd.read_csv(file_location + 'Raw_otree/all_apps_wide_e1314ij0.csv')
prolific_data = prolific_data.loc[:, ['participant.code', 'participant.label']]

len(prolific_data['participant.label'].unique().tolist()) # 46 participants  
...
nocode_participants_code = prolific_data[prolific_data['participant.label'].isin(nocode_label)]
grouped_participants_code = prolific_data[prolific_data['participant.label'].isin(grouped_participants_label)]
...
grouped_participants_last_page = page_time[page_time['participant_code'].isin(grouped_participants_code['participant.code'])][['participant_code', 'last_page_name']]
...
grouped_participants_last_page = grouped_participants_last_page.drop_duplicates(subset=['participant_code'])
...
grouped_participants_last_page = grouped_participants_last_page.merge(grouped_participants_code, left_on='participant_code', right_on='participant.code', how='left')
...
plt.show()
plt.savefig('average_time_differences_all_pages_e1314ij0.png')
plt.close()
...
len(waitbase_page_time['participant_code'].unique().tolist() )# 18 waiting participants + 10 base pay participants = 30 participants



# Filter out the ones getting grouped
# if one participant_code has multiple Discussion pages, mark them as "grouped" in a new column called "grouped"
waitpart_page_time['grouped'] = waitpart_page_time.groupby('participant_code')['page_name'].transform(lambda x: 'grouped' if (x == 'Discussion').sum() > 1 else 'not_grouped')

# Slice out the grouped participants into a new DataFrame
grouped_page_time = waitpart_page_time[waitpart_page_time['grouped'] == 'grouped']

# Slice out the not grouped participants into a new DataFrame
not_grouped_page_time = waitpart_page_time[waitpart_page_time['grouped'] == 'not_grouped']

min_time = not_grouped_page_time['epoch_time_completed'].min()
not_grouped_page_time['time_from_start'] = not_grouped_page_time['epoch_time_completed'] - min_time


# Bin time into 10s intervals
not_grouped_page_time['time_bin'] = (not_grouped_page_time['time_from_start'] // 10) * 10

# Get unique participant codes and page names
participants = not_grouped_page_time['participant_code'].unique() 
len(participants) # 10 participants not grouped
...
relevant_page_names = ['InitializeParticipant',
                        'Introduction',
                        'Demographics',
...
otree_data = pd.read_csv(file_location + 'Raw_otree/all_apps_wide_e1314ij0.csv')
...
# Exclude pages from the data
not_grouped_page_time = not_grouped_page_time[not_grouped_page_time['page_name'].isin(relevant_page_names)]

plt.figure(figsize=(20, 6))
for i, participant in enumerate(participants):
    df_part = not_grouped_page_time[not_grouped_page_time['participant_code'] == participant]
...
min_bin = int(not_grouped_page_time['time_bin'].min())
max_bin = int(not_grouped_page_time['time_bin'].max())
...
plt.tight_layout()
plt.savefig('participant_page_transitions_over_time_gneiwiau_waitonly.png')
plt.show()
...
# Mark the participants who are grouped
waitbase_page_time['grouped'] = waitbase_page_time.groupby('participant_code')['page_name'].transform(lambda x: 'grouped' if (x == 'Discussion').sum() > 1 else 'not_grouped')
...
len(waitbase_participants) # 28 participants
# Get the participant codes and their answers in Scenario
...
len(waitbase_page_time_clean['participant_code'].unique().tolist()) # 28 participants
...
# Debug to see each page's time converted for participant_code e7yqt270
waitbase_page_time_clean[waitbase_page_time_clean['participant_code'] == 'e7yqt270'][['page_name', 'time_converted']].drop_duplicates()
...
plt.legend(handles=handles, title='Page Name', bbox_to_anchor=(1.05, 1), loc='upper left')
plt.tight_layout()
plt.savefig('waitingandbasepay_participant_page_transitions_over_time_e1314ij0.png')
plt.show()




###### Grouped participants analysis ######
# Filter the page_time DataFrame to only include waiting participants
grouped_page_time = page_time[page_time['participant_code'].isin(grouped_participants_last_page['participant_code'])]

min_time = grouped_page_time['epoch_time_completed'].min()
grouped_page_time['time_from_start'] = grouped_page_time['epoch_time_completed'] - min_time

# Bin time into 10s intervals
grouped_page_time['time_bin'] = (grouped_page_time['time_from_start'] // 10) * 10

# Get unique participant codes and page names
grouped_participants = grouped_page_time['participant_code'].unique() 
len(grouped_participants) # 8 participants

# Get the participant codes and their answers in Scenario
grouped_otree_data = otree_data[otree_data['participant.code'].isin(grouped_participants_last_page['participant_code'].unique().tolist())]

grouped_scenario_answers = grouped_otree_data[['participant.code','presurvey.1.player.response']]
...
grouped_page_time_clean = grouped_page_time[grouped_page_time['page_name'].isin(relevant_page_names)]
len(grouped_page_time_clean['participant_code'].unique().tolist()) # 8 participants


# Debug: seeing the time in UTC format
grouped_page_time_clean['time_converted'] = pd.to_datetime(grouped_page_time_clean['epoch_time_completed'], unit='s')

# Debug to see each page's time converted for participant_code e7yqt270
grouped_page_time_clean[grouped_page_time_clean['participant_code'] == 'e7yqt270'][['page_name', 'time_converted']].drop_duplicates()

len(grouped_participants)
len(grouped_page_time_clean['participant_code'].unique().tolist()) # 8 participants
len(grouped_scenario_answers['participant.code'].unique().tolist()) # 8 participants
...
for i, participant in enumerate(grouped_participants):
    df_part = grouped_page_time_clean[grouped_page_time_clean['participant_code'] == participant]
    # Get the scenario answer for this participant
    scenario_row = grouped_scenario_answers[grouped_scenario_answers['participant.code'] == participant]
    if not scenario_row.empty:
        answer = scenario_row['presurvey.1.player.response'].values[0]
        if answer == -1:
            line_color = '#097969'
...
# Set x-ticks to show every 2 minutes (120s)
min_bin = int(grouped_page_time_clean['time_bin'].min())
max_bin = int(grouped_page_time_clean['time_bin'].max())
step = 120  # 2 minutes in seconds
xticks = np.arange(min_bin, max_bin + step, step)
xtick_labels = [str(int(x // 60)) for x in xticks]  # show as minutes
plt.xticks(xticks, xtick_labels, rotation=45)
plt.xlabel('Time (minutes, binned every 2 min)')
# Set y-ticks to participant codes
plt.yticks(range(len(grouped_participants)), grouped_participants)
plt.ylabel('Participant Code')
plt.title('Participant Page Transitions Over Time')

# Create a legend for page_name colors (below the plot, as wide as the plot)
handles = [plt.Line2D([0], [0], marker='o', color='w', label=name, markerfacecolor=page_name_to_color[name], markersize=10) for name in relevant_page_names]
# add label for line colors
line_labels = ['Scenario Answer: -1 Against', 'Scenario Answer: 1 For']
line_colors = ['#097969', '#400080'] 
...
plt.savefig('grouped_participant_page_transitions_over_time_e1314ij0.png')
plt.show()
...
# Use the waitbase scenario_answers
waitbase_scenario_answers

# Note: there are one nocode participants who answered the scenario question but is not in the base pay participants
nocode_participants_code

# Get the participant codes and their answers in Scenario
# waiting, base pay, and nocode participants
waiting_otree_data = otree_data[otree_data['participant.code'].isin(waiting_participants_code['participant.code'])]
waiting_scenario_answers = waiting_otree_data[['participant.code','presurvey.1.player.response']]

base_pay_otree_data = otree_data[otree_data['participant.code'].isin(base_pay_participants_code['participant.code'])]
base_pay_scenario_answers = base_pay_otree_data[['participant.code','presurvey.1.player.response']]

nocode_otree_data = otree_data[otree_data['participant.label'] == nocode_participants_code['participant.label'].values[0]]
nocode_scenario_answer = nocode_otree_data['presurvey.1.player.response']
...
plt.legend()
plt.tight_layout()
plt.savefig('scenario_answers_distribution_stacked_e1314ij0.png')
plt.show()


#### Analysis of the discussion round ####