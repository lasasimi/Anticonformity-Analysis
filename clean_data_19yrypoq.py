...
filename = 'all_apps_wide_' + session_id + '.csv' # change file name as needed
df_raw = pd.read_csv(data_raw + filename) # change file name to current file

# get only participants who reached the end of the experiment
df_raw = df_raw[df_raw['participant._current_page_name'] == 'Feedback'].reset_index(drop=True)
# but this excludes participants who were timed out in the mock app
# add those participants back in
df_raw_nopay = df_raw[df_raw['participant.reason'] == 'You have been timed out for inactivity.'].reset_index(drop=True)
df_raw = pd.concat([df_raw, df_raw_nopay]).reset_index(drop=True)
len(df_raw) # 8 participants
...
cleanfilename = 'clean_long_format_' + session_id + '.csv'
